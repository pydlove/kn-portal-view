...
import {deleteRkMenu, getRkMenuPage} from '@/api/ruankao/menu/menu'
...
const fetchData = async (current?: number, pageSize?: number) => {
  ...
  try {
    ...
    const res: any = await getRkMenuPage({
...
const openAddSubMenuModal = (parentMenu: RkMenu) => {
  if (addMenuModal.value) {
...
const editingMenu = ref<RkMenu | null>(null)

const openEditMenuModal = (menu: RkMenu) => {
...
// 删除菜单
const handleDelete = async (menu: RkMenu) => {
  try {
    await deleteRkMenu(menu.id!)
    await fetchData()
  } catch (error) {
