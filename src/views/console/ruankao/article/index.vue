...
import {deleteRkArticle, getRkArticlePage} from "@/api/ruankao/article/article";
import {message, Modal} from "ant-design-vue";
import {getRkAllMenus} from "@/api/ruankao/menu/menu";
...
// 文章列表
const articleList = ref<RkArticleVO[]>([])
...
// 处理删除
const handleDelete = (row: RkArticleVO) => {
  Modal.confirm({
    title: '删除确认',
...
const editingArticle = ref<RkArticleVO | null>(null)
const openEditArticleModal = (record: RkArticleVO) => {
...
const handlePreview = (record: RkArticleVO) => {
...
const fetchArticleList = async () => {
  ...
  try {
    ...
    const res: any = await getRkArticlePage(params)
...
const fetchMenus = async () => {
  try {
    menuOptions.value = await getRkAllMenus()
  } catch (error) {
    console.error('getRkAllMenus', error)
  }
}
