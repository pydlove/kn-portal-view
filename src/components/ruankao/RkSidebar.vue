...
import {getRkSubMenus} from "@/api/ruankao/home/home";
...
const fetchMenuData = async () => {
  try {
    ...
    articleMenuList.value = await getRkSubMenus({parentMenuId: activeMenuIdRef.value})
    console.log(  articleMenuList.value)
...
const checkHasSelectedRkArticle = () => {
  const storedArticle = sessionStorage.getItem('selectedRkArticle');
  if (storedArticle) {
    const article = JSON.parse(storedArticle);
