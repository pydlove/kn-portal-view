  ...
  () => {
    ...
    const login = async (params: API.LoginDto) => {
      try {
        ...
        const userPwd = encrypt(params.userPwd, randomCode)
        const res = await onLogin({ ...params, userPwd, randomId })
    ...
    console.log(menusMap, 'menusMap1')
